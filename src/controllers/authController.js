...
exports.createUser = asyncHandler(async (req, res, next) => {
    ...
    if (password != passwordConfirmation) {
        next(new ErrorResponse('Passwords are not matched!', 201));
    }
    ...
    return res.status(200).json({ success: true, message: 'You successfully created a user!' });
});
...
exports.activateAccount = asyncHandler(async (req, res, next) => {
    ...
    return res.redirect(301,'http://localhost:3000/sign-in');
});
...
exports.loginUser = asyncHandler(async (req, res, next) => {
    ...
    if (!email || !password) {
        return next(new ErrorResponse('Please provide email and password!', 201));
    }

    const user = await User.findOne({ email }).select('email password status');

    if (!user) {
        return next(new ErrorResponse('Provided email is not correct!', 202));
    }

    if (user.status === 'PENDING') {
        return next(new ErrorResponse('Please verify your account!', 203));
    }

    const isMatch = user.isMatchedPassword(password);

    if (!isMatch) {
        return next(new ErrorResponse('Password is incorrect!', 206));
    }
